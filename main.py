...
def main():
    ...
    def getCurrentTable():
        return MenuListBox.curselection()[0]
    ...
    def mainLoop():
        ...
        tables = tableGen(cursor.execute("SELECT count(*) FROM tables").fetchone()[0])
        for name in tables:
            if name not in MenuListBox.get(0, "end"):
                MenuListBox.insert(MenuListBox.size(), name)
    ...
    MenuListBox = Listbox(frameMove, height = 10, width = 10, selectmode = SINGLE, font = s)




    MainListBox = Listbox(frameMiddle, height = 50, width = 100, selectmode = "SINGLE")
    MainListBox.bind("<<ListboxSelect>>", listRouter)
    ...
    NewButton = Button(frameActions, text = "New List", command = newListWindow, font = s)
    ...
    MenuListBox.grid(column = 0, row = 1, padx = 5)
